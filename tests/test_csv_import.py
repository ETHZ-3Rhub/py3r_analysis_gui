"""Tests for the pure matching engine in app/csv_matching.py."""
...
# ── preprocess_tokens / tokenize ────────────────────────────────────────────


def _pp(s, *, nonalpha=True, boundary=None, case=True, zeros=False, ignore=None):
    return preprocess_tokens(s, nonalpha, boundary or [], case, zeros, ignore or [])
...
def testtokenize_keeps_raw_spans():
    # normalised value + the span of the RAW token (so highlighting hits raw text)
    assert tokenize("OFT01_8", True, [], False, True, []) == [("oft1", 0, 5), ("8", 6, 7)]


# ── find_match — returns (handle_indices, stem_indices) ──────────────────────


def _match_vals(handle, stem, min_tokens, order, unint):
    """Map find_match's index result back to (handle_values, stem_values)."""
    res = find_match(handle, stem, min_tokens, order, unint)
    if res is None:
        return None
    h_idx, s_idx = res
...
def test_find_set_missing_one_fails_all():
    assert find_match(["oft1", "8"], ["oft1", "9"], 0, False, False) is None
...
def test_find_order_subsequence_with_gaps():
    assert find_match(["a", "b", "c"], ["a", "x", "b", "y", "c"], 0, True, False) == (
        [0, 1, 2],
        [0, 2, 4],
    )
...
def test_find_order_wrong_order_fails():
    assert find_match(["a", "b"], ["b", "a"], 0, True, False) is None
...
def test_find_window_interrupted_fails():
    assert find_match(["dog", "cat"], ["dog", "moose", "cat"], 0, False, True) is None
...
def test_find_subarray_block_in_noise():
    ...
    assert find_match(handle, ["oft1", "8"], 2, True, True) == ([3, 4], [0, 1])


def test_find_subarray_order_matters():
    assert find_match(["a", "b"], ["b", "a"], 2, True, True) is None
...
def test_find_empty_handle_is_none():
    assert find_match([], ["a", "b"], 0, False, False) is None


def test_find_threshold_above_handle_length():
    assert find_match(["a"], ["a"], 2, False, False) is None
...
def testcompute_matches_clean(tmp_path):
    files = [tmp_path / "OFT1_1.csv", tmp_path / "OFT1_2.csv"]
    for f in files:
        f.touch()
    rows = _rows("OFT1_1", "OFT1_2")
    result = compute_matches(rows, "handle", ["treatment"], files, **_cfg())
    assert len(result.clean_matches) == 2
    assert result.conflicts == []
    assert result.files_not_in_csv == []


def testcompute_matches_row_conflict(tmp_path):
...
def testcompute_matches_unmatched_file(tmp_path):
...
def testcompute_matches_zero_tolerance(tmp_path):
    files = [tmp_path / "OFT1_1.csv"]
    files[0].touch()
    rows = _rows("OFT01_01")
    result = compute_matches(rows, "handle", ["treatment"], files, **_cfg(tolerate_zeros=True))
    assert len(result.clean_matches) == 1


def testcompute_matches_multi_column_group(tmp_path):
    files = [tmp_path / "OFT1_1.csv", tmp_path / "OFT1_2.csv"]
    for f in files:
        f.touch()
    rows = [
    ...
    result = compute_matches(rows, "handle", ["treatment", "timepoint"], files, **_cfg())
    groups = {m.group_name for m in result.clean_matches}
    assert groups == {"control_pre", "stressor_pre"}


def testcompute_matches_spans_only_matched_block(tmp_path):
...
def testcompute_matches_id_spans_map_into_id_val(tmp_path):
    f = tmp_path / "rec_OFT1_8.csv"
    f.touch()
    rows = _rows("OFT1_8")
    result = compute_matches(rows, "handle", ["treatment"], [f], **_cfg())
    m = result.clean_matches[0]
...
def _mk(path, id_val="x", group="G1"):
    return Match(path=path, id_val=id_val, id_spans=[], name_spans=[], group_name=group)


def testbuild_result_groups_clean(tmp_path):
    f1, f2 = tmp_path / "a.csv", tmp_path / "b.csv"
    matches = [_mk(f1, "a", "G1"), _mk(f2, "b", "G2")]
    groups = build_result_groups(matches, [])
    assert set(groups.keys()) == {"G1", "G2"}
    assert groups["G1"] == [f1]


def testbuild_result_groups_resolved_conflict(tmp_path):
    f1, f2 = tmp_path / "a.csv", tmp_path / "b.csv"
    conflict = Conflict(
        label="x matches 2 files", options=[_mk(f1), _mk(f2)], selection=frozenset({0})
    )
    groups = build_result_groups([], [conflict])
    assert groups["G1"] == [f1]


def testbuild_result_groups_excluded_conflict(tmp_path):
    f1 = tmp_path / "a.csv"
    conflict = Conflict(label="x matches 1 file", options=[_mk(f1)], selection=frozenset())
    groups = build_result_groups([], [conflict])
    assert groups == {}


def testbuild_result_groups_unresolved_conflict_excluded(tmp_path):
    f1 = tmp_path / "a.csv"
    conflict = Conflict(label="x", options=[_mk(f1)], selection=None)
    groups = build_result_groups([], [conflict])
    assert groups == {}
