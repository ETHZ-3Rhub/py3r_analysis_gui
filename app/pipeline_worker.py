...
resolved config and kwargs from *payload_path*, imports the config's entry
function, and calls it. stdout/stderr pass straight through to the parent's pipe
so PipelineRunner can stream them; running this off the main process means
"Cancel" can kill it outright without risking a hung GUI.
"""
